...
class WC_Gateway_VietQR extends WC_Payment_Gateway
{
     ...
     */
    private static $URL_BANKS_LIST = "https://api.vietqr.io/v2/banks";
    private static $URL_QRCODE = "https://api.vietqr.io/v2/generate";
    public function __construct()
    {
        ...
        $this->icon = apply_filters(
            "woocommerce_icon_vietqr",
            plugins_url("../assets/img/vietqr.png", __FILE__)
        );
        $this->method_title = __(
            "Direct bank transfer (Scan VietQR)",
            "vietqr"
        );
        $this->method_description = __(
            "Take payments by scanning QR code with Vietnamese banking App. Supported by most major banks in Vietnam",
            "vietqr"
        );
        ...
        if (
            !$this->banks_list ||
            !$vietqr_banks_expired ||
            $vietqr_banks_expired < time() ||
            $vietqr_banks_expired > time() + $CACHE_LIFE
        ) {
            //call api.
        ...
        if (strpos($this->description, "Hơn 14 ngân hàng Việt Nam") !== false) {
            $this->description = str_replace(
                "Hơn 14 ngân hàng Việt Nam",
                "Hầu hết ngân hàng Việt Nam",
                $this->description
            );
        ...
        $this->account_details = get_option("woocommerce_vietqr_accounts", [
            [
        ...
        add_action("woocommerce_thankyou_vietqr", [$this, "thankyou_page"]);
        // Customer Emails.
        add_action(
        ...
        add_action("woocommerce_api_vietqr/developer", [
            $this,
            "vietqr_developer_payment_handler",
        ]);
        wp_enqueue_style(
            "vietqr-custome-styles",
    ...
    public function init_form_fields()
    {
        ...
        $this->form_fields = [
            ...
            "prefix" => [
                ...
                "type" => "text",
                "description" => __(
                    "Prefix used to combine with order code to create money transfer content, Set rules: no spaces, no more than 15 characters and no special characters. Violations will be deleted",
                    "vietqr"
                ),
    ...
    public function generate_account_details_html()
    {
		...
		<tr valign="top">
			...
			<td class="forminp" id="vietqr_accounts">
				<select id='vietqr_bank_selector2' style='width: 100%' name="vietqr_bank_name[0]">
					<?php foreach ($this->banks_list["data"] as $bank) {
         $selected = "";
		...
		<tr valign="top">
			<th scope="row" class="titledesc"><?php echo __(
       "Account name",
       "vietqr"
   ); ?>:</th>
			<td class="forminp" id="vietqr_accounts">
				<?php echo '<input type="text" value="' .
        esc_html($this->account_details[0]["account_name"]) .
        '" name="vietqr_account_name[0]" />'; ?>
			</td>
		</tr>
		<tr valign="top">
			<th scope="row" class="titledesc"><?php echo __(
       ...
       "vietqr"
			...
			<td class="forminp" id="vietqr_accounts">
				<?php echo '<input type="text" value="' .
        esc_html($this->account_details[0]["account_number"]) .
        '" name="vietqr_account_number[0]" />'; ?>
			</td>
			</td>
		</tr>
		<tr valign="top" style="<?php if (!$is_claim_show) {
      echo "display: none;";
  } ?>">
			<th scope="row" class="titledesc"><?php echo __(
       "Developer ID",
       "vietqr"
        ...
        $this->developer_id
    ); ?>" name="vietqr_developer_id[0]" style="max-width: 150px;"/>
				</br>
				<label for="bank_transfer" style="font-size: 11px; font-style: oblique;margin-top: 10px;"><?php echo __(
        "Chúng tôi sẽ xét duyệt và gửi quà qua Momo bạn nếu đủ điều kiện",
...
<?php return ob_get_clean();
    ...
    public function save_account_details()
    {
        ...
        if (
            isset($_POST["vietqr_account_name"]) &&
            isset($_POST["vietqr_account_number"])
        ) {
            $account_names = wc_clean(
                wp_unslash($_POST["vietqr_account_name"])
            );
            $account_numbers = wc_clean(
                wp_unslash($_POST["vietqr_account_number"])
            );
        ...
        if (isset($_POST["vietqr_account_name"])) {
            $_POST["woocommerce_vietqr_prefix"] = $this->clean_prefix(
                $_POST["woocommerce_vietqr_prefix"]
            );
        ...
        if (isset($_POST["vietqr_developer_id"])) {
            ...
            $this->update_option(
                "developer_id",
                $_POST["woocommerce_vietqr_developer_id"]
            );
            $this->developer_affiliate(
                $_POST["woocommerce_vietqr_developer_id"]
            );
        }
    }
    ...
    public function thankyou_page($order_id)
    {
        if ($this->instructions) {
            echo wp_kses_post(
                wpautop(wptexturize(wp_kses_post($this->instructions)))
            );
        }
        //	echo 'ok';
    ...
    public function email_instructions(
        ...
        if (
            !$sent_to_admin &&
            "vietqr" === $order->get_payment_method() &&
            $order->has_status("on-hold")
        ) {
            if ($this->instructions) {
                echo wp_kses_post(
                    wpautop(wptexturize($this->instructions)) . PHP_EOL
                );
            }
    ...
    private function bank_details($order_id = "", $is_sent_email = false)
    {
        ...
        foreach ($vietqr_accounts as $vietqr_account) {
            ...
            if ($vietqr_account->account_name) {
                $account_html .=
                    '<h3 class="wc-vietqr-bank-details-account-name">' .
                    wp_kses_post(wp_unslash($vietqr_account->account_name)) .
                    ":</h3>" .
                    PHP_EOL;
            }

            $account_html .=
                '<ul class="wc-vietqr-bank-details order_details vietqr_details">' .
                PHP_EOL;
			...
			<!-- QR TITILE-->';
                ...
                if ($qrcode_url) {
                    $showPayment .=
                        '
						<p class="wc-vietqr-bank-details-heading" style="text-align: center;">' .
                        __("Bank transfer QR code", "vietqr") .
                        '</p>
						<!-- QR IMAGE HERE-->
			...
			<div style="">
				<div id="qrcode" style="text-align: center;">
					<img src="' .
                        esc_html($qrcode_url) .
                        '"  alt="VietQR QR Image" width="400px" />
				</div>

				<!--buton download on email.-->
				<a style="max-width:200px; margin: 0 auto;background-color: limegreen; color:white; display:' .
                        ($is_sent_email ? "block" : "none") .
                        '" href="' .
                        esc_html($qrcode_page) .
                        '" target="_blank" >
					Tải QR Code             
				</a>
				
				<a id="downloadQR" download="vietqr_' .
                        esc_html($account_fields["account_number"]["value"]) .
                        '.jpg"  href="' .
                        esc_html($qrcode_url) .
                        '">
					...
					' .
                        __(
                            "Then open the banking App and scan the transfer QR",
                            "vietqr"
                        ) .
                        '
					</i></span>
			...
			<section class="woocommerce-vietqr-bank-details">
				...
				<!-- BANK DETAIL TITLE-->
				<h2 class="wc-vietqr-bank-details-heading" style="text-align: center;">' .
				...
				<h4 style="color: black;">' .
                    sprintf(
                        __(
                            "Please transfer the correct content <bordermemo>%s</bordermemo> for we can confirm the payment",
                            "vietqr"
                        ),
                        esc_html($this->prefix . $order_id)
                    ) .
                    '</h4>
				...
				<tbody>
				<tr class="" >
						<td class="text-right width-column-70"  style="text-align: left;">
							<span style="color: black;">' .
                    __("Account name", "woocommerce") .
                    ':</span>
							<br>
							<div>
								<strong id="account_name" style="color: black;">' .
                    esc_html($account_fields["account_name"]["value"]) .
                    '</strong>
								<br>
							</div>
						</td>
						<td class="text-left payment-instruction width-column-30" style="text-align: right;">
							<div>
								<button class="copy-button" data-copy-target="account_name">Sao chép</button>
								<br>
							</div>
						</td>
					</tr>
					<tr class="" style="background-color:#FBFBFB;">
						<td class="text-right width-column-70"  style="text-align: left;">
							<span style="color: black;">' .
                    __("Account number", "vietqr") .
                    ':</span>
							<br>
							<strong id="account_number"  style="color: black;">' .
                    esc_html($account_fields["account_number"]["value"]) .
                    '</strong>
						</td>
						<td class="text-left payment-instruction width-column-30" style="text-align: right;">
							<div>
								<button class="copy-button" data-copy-target="account_number">Sao chép</button>
								<br>
							</div>
						</td>
					</tr>
                    ...
                    ':</span>
							<br>
							<div>
								<strong id="bank_name"  style="color: black;">' .
                    esc_html($account_fields["bank_name"]["value"]) .
                    '</strong>
								<br>
							</div>
						</td>
						<td class="text-left payment-instruction width-column-30" style="text-align: right;">
							<div>
								<button class="copy-button" data-copy-target="bank_name">Sao chép</button>
								<br>
							</div>
						</td>
					</tr>
                    ...
                    ':</span>
							...
							<div ng-switch-when="vcb" class="ng-scope">
								<strong id="amount" style="color: black;">' .
                    esc_html($account_fields["amount"]["value"]) .
                    ' <sup>vnđ</sup></strong>
								<br>
							</div>
						</td>
						<td class="text-left payment-instruction width-column-30" style="text-align: right;">
							<div>
								<button class="copy-button" data-copy-target="amount">Sao chép</button>
								<br>
							</div>
						</td>
					</tr>
                    ...
                    '*:</span>
							<br>
							<strong id="memo" style="text-align: left;color: red;">
							' .
                    ...
                    '
						...
						<td class="text-left payment-instruction width-column-30" style="text-align: right;">
							<div>
								<button class="copy-button" data-copy-target="memo">Sao chép</button>
								<br>
							</div>
						</td>
					</tr>
...
';
    ...
    public function process_payment($order_id)
    {
        ...
        if ($order->get_total() > 0) {
            ...
            $order->update_status(
                apply_filters(
                    "woocommerce_vietqr_process_payment_order_status",
                    "on-hold",
                    $order
                ),
    ...
    public function get_banks_list()
    {
        ...
        $response = wp_remote_get($url, [
            "headers" => [
                ...
                "x-client-id" => get_site_url(),
                "content-type" => "application/json",
                "referer" => home_url(add_query_arg([$_GET], $wp->request)),
            ],
